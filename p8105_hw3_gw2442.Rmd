...
This is a two-panel plot showing the average max temperatur ein January and July in each station across years: 
```{r}
max_temp_jan = 
  problem_3 %>%
  filter(month == 01) %>%
...
max_temp_july = 
  ...
  ggplot(data = max_temp_jan, aes(x = year, y = mean_tmax, color)) +
  ...
  ggplot(data = max_temp_july, aes(x = year, y = mean_tmax)) + 
  geom_point(alpha = 0.5) +
  geom_smooth(se = FALSE) +
  scale_y_continuous(
    limits = c(-20, 40)) +
